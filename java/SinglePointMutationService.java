...
@Slf4j
@Service
public class SinglePointMutationService  {
	...
	public Map<String,Object> singleMutation(PointMutationEntity pointMutation){
...
//		Map<String,Object> checkMap = checkField(params); 
...
//		}
		...
		if(Mut.length() >= LengthStore) {
			PrimerDesign(Mut3);
			OuterFTm = PrimerFTm;
			result.put("PrimerFvalue", Spit5seq + PrimerF);
			result.put("PrimerFbefore", "5'-");
			result.put("PrimerFafter", "- 3'");
			result.put("PrimerFabove1", "单点突变正向扩增引物为：");
			result.put("PrimerFabove1_en", "The forward amplification primer sequence for single-point mutation:");
			...
			result.put("PrimerRabove1", "单点突变反向扩增引物为：");
			result.put("PrimerRabove2", "基因特异性引物Tm计算值为："+PrimerRTm+"℃");
			result.put("PrimerRabove1_en", "The reverse amplification primer sequence for single-point mutation:");
			...
			PrimerDesign(Mut3);
			FTemp = PrimerF;
			if((FTemp.length()-Spit3) < 15) {
				FTemp = Mut3.substring(0, 15 + Spit3);
			}
			OuterFTm = Tm_Cal(FTemp);
			result.put("PrimerFvalue", Spit5seq + FTemp);
			result.put("PrimerFbefore", "5'-");
			result.put("PrimerFafter", "- 3'");
			result.put("PrimerFabove1", "单点突变正向扩增引物为：");
			result.put("PrimerFabove1_en", "The forward amplification primer sequence for single-point mutation:");
			...
			PrimerDesign(Mut5);
			RTemp = PrimerR;
			if((RTemp.length()-Spit5) < 15) {
				RTemp = Mut5.substring(Mut5.length() - 15 - Spit5);
				RTemp = RC(RTemp);
			}
			OuterRTm = Tm_Cal(RTemp);
			result.put("PrimerRvalue", Spit3seq + RTemp);
			result.put("PrimerRbefore", "5'-");
			result.put("PrimerRafter", "- 3'");
			result.put("PrimerRabove1", "单点突变反向扩增引物为：");
			result.put("PrimerRabove2", "基因特异性引物Tm计算值为："+OuterRTm+"℃");
			result.put("PrimerRabove1_en", "The reverse amplification primer sequence for single-point mutation:");
			result.put("PrimerRabove2_en", "Gene Specific Primer Tm Value:"+OuterRTm+"℃");
		...
		sequence(result.get("PrimerFvalue").toString().substring(0, LengthStore));
		if(((double)(numA) + (double)(numT)) / (double)(LengthStore) > 0.7d) {
		...
		}else if(((double)(numA) + (double)(numT)) / (double)(LengthStore) < 0.3d) {
...
//					Double tmp1 = Math.abs(i+1 - (GC.length+1d)/2d);
//					Double tmp2 = Math.abs(NiceGCNum+1 - (GC.length+1d)/2d);
